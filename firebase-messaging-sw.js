...
messaging.onBackgroundMessage((payload) => {
  ...
  const image =
    ...
    "/img/2bkc.jpg";
  const link =
    payload.data?.link || payload.notification?.click_action || "/#news";
  ...
  const notificationOptions = {
    ...
    renotify: true,
    vibrate: [200, 100, 200], // การสั่นช่วยกระตุ้นให้ Android ยอมให้เด้ง Pop-up
    priority: 2, // ระดับสูงสุด (Max Priority)
    requireInteraction: true, // ให้การแจ้งเตือนค้างไว้จนกว่าผู้ใช้จะกดหรือปัดทิ้ง
    data: {
      url: link,
    },
...
self.addEventListener("notificationclick", function (event) {
  ...
  const targetUrl = new URL(
    event.notification.data?.url || "/#news",
